...
PATTERNS = [
    (re.compile(r"ModuleNotFoundError|No module named", re.I), "Python dependency or import path issue"),
    ...
    (re.compile(r"ECONNREFUSED|connection refused", re.I), "Service or server is not reachable"),
    (re.compile(r"\bENOTFOUND\b|getaddrinfo", re.I), "DNS, host, or network configuration issue"),
    (re.compile(r"Cannot find module", re.I), "Node.js dependency or path issue"),
    (re.compile(r"command not found|not recognized", re.I), "Missing command or PATH configuration issue"),
]
...
def analyze_error_text(text: str, source: str = "inline input") -> DebugAnalysis:
    ...
    return DebugAnalysis(
        ...
        likely_causes=sorted(set(likely_causes)) or ["No known error pattern matched. More context may be needed."],
...
def _build_next_steps(likely_causes: list[str], signals: list[str]) -> list[str]:
    ...
    if "path" in joined or "command" in joined:
        steps.append("Verify the command exists and that the project setup instructions were followed.")
